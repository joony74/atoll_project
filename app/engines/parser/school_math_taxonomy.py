...
TOPIC_SPECS: tuple[TopicSpec, ...] = (
    TopicSpec("calculus_limit", "극한", "high", (r"\blim\b|극한|연속",)),
    TopicSpec("calculus_integral", "적분", "high", (r"∫|적분|넓이.*함수",)),
    TopicSpec("calculus_derivative", "미분", "high", (r"미분|도함수|접선|극대|극소|f'\(|f′\(",)),
    TopicSpec("trigonometry", "삼각함수", "high", (r"\b(?:sin|cos|tan)\b|삼각함수|사인|코사인|탄젠트|π|\\pi\b",)),
    TopicSpec("log_sequence", "로그 수열", "high", (r"sequence_log_product|수열.*log|log.*수열",)),
    TopicSpec("logarithm", "지수·로그", "high", (r"\blog\b|로그|지수함수|밑",)),
    TopicSpec("sequence", "수열", "high", (r"수열|등차|등비|an\b|a_n|시그마|Σ|sum",)),
    TopicSpec("probability", "확률", "middle_high", (r"확률|경우의 수|순열|조합|P\(|C\(|(?:\d+|[nNrR])\s*!",)),
    ...
    TopicSpec(
        ...
        "계산",
        "elementary",
        (r"\d+\s*[+\-*/]\s*\d+|값은|계산", r"모으기|가르기|빈칸|알맞은", r"몇\s*(?:명|개)|더\s*필요한|가지고\s*있었"),
    ),
)
...
def classify_school_math_topic(text: str, expressions: Iterable[str] = ()) -> tuple[str, float]:
    ...
    if not joined.strip():
        return "unknown", 0.0
    if re.search(r"\^\(\s*1\s*/\s*[23]\s*\)", joined):
        return "radical_power", 0.75
    if re.search(r"모으기|가르기", joined) and re.search(r"빈칸|알맞은|answer\s*=", joined, flags=re.IGNORECASE):
        return "arithmetic", 0.75
    if re.search(r"answer\s*=", joined, flags=re.IGNORECASE) and re.search(
        r"몇\s*(?:명|개|씩)|더\s*필요|가지고\s*있었|기념품|규칙|빈칸|알맞은", joined
    ):
        return "arithmetic", 0.75
